...
@realtimealarm.route('/DevFailure', methods=['post'])
def DevFailure():
    token = request.form.get("token")
    # starttime = request.form.get("starttime")
    # endtime = request.form.get("endtime")
    # stationid = request.form.get("stationid")
    current_app.logger.info('VariousExceptAlarmStatistics   token:{}'.format(token))
    #
    # req = ReqResult()
    # # 当没有开始时间时,直接获取失败
    # if not all([starttime, ]):
    ...
    #         time_list = nowTime.split(' ')
    #         endtime = time_list[0]
    select_camera_failure_sql = 'select m_camera.camera_id, m_camera.sts from m_taskalarm left join m_taskalarmtype on m_taskalarm.id = m_taskalarmtype.errorid left join m_task on m_taskalarmtype.task_id = m_task.task_id left join m_camera on m_task.camera_id = m_camera.camera_id where m_camera.sts = 0 group by m_camera.camera_id, m_camera.sts'
    select_camera_failure = db.select_db(select_camera_failure_sql)
    print(len(select_camera_failure))
    req = ReqResult()
    req.code = 0
    req.data = {
        'count':'{}'.format(len(select_camera_failure)),
    }
    return json.dumps(req.__dict__)
...
'''
    ...
    16.导出异常设备明细
    ExportAbnormalDevInfo

'''


@realtimealarm.route('/ExportAbnormalDevInfo', methods=['post'])
def ExportAbnormalDevInfo():

    '''
    设备故障,摄像头故障,导出excel表
    :return:  Export Success or failure
    '''
    ...
    # ws1 = wb.create_sheet('摄像头异常', 0)
    # ws2 = wb.create_sheet('设备异常', 1)
    #
    # # 标题
    ...
    #     ws2['A1'] = 'juankuan'
    #     ws2['E1'] = '中国'
    #     ws2['F1'] = '2020-02-27-09-51-02'
    #
    #     # -----------------------------------------------
    #
    #     wb.save('E:\\WORK\\code\\p\\DetectVideoServer\\设备异常.xlsx')
...
def ExportData():
    ...
    try:
        ...
        # -----------------------------------------------
        file_name = str(datetime.date.today()) + str(time.time())
        file_name_path = 'E:\\WORK\\code\\p\\DetectVideoServer\\{}-设备异常.xlsx'.format(file_name)
...
def AlarmTrend():
    ...
    # 月
    if int(time_type) == 2:
        month_select_sql_10 = 'SELECT DATE_FORMAT(@date := DATE_ADD(@date, INTERVAL - 1 MONTH),"%Y-%m") as month FROM ( SELECT @date := DATE_ADD("2020-12-16", INTERVAL + 1 MONTH) FROM  m_taskalarm  LIMIT 10 ) time;'.format()
